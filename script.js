document.addEventListener("DOMContentLoaded", () => {
    ...
    const youtubeUrl = id => (id ? `https://www.youtube.com/watch?v=${id}` : "");
    ...
    const renderList = (container, category, videoData) => {
        ...
        list.forEach(item => {
            ...
            const link = item.link || youtubeUrl(item.id);
            if (link) {
    ...
    const bindModal = () => {
        document.querySelectorAll(".video-card[data-link]").forEach(card => {
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (wishForm && wishMarquee) {
        ...
        const API_URL = "https://script.google.com/macros/s/AKfycbzIXp7RX1I9IwbepvrXug34LzX0J2kE7-R4iwgH-zy83cWv6wlkIjUazkEa-JyIIA-s/exec";
